...
<body class="bg-purple-50 text-gray-800">
  ...
  <section class="relative overflow-hidden">
    <div id="banner-slides" class="relative w-full h-[400px] rounded-xl overflow-hidden">
      <div class="slide absolute inset-0 fade active">
        ...
        <div class="absolute inset-0 flex flex-col items-center justify-center text-white text-center">
          <h1 class="text-4xl font-bold mb-2 drop-shadow-lg">Thức ăn cho thú cưng </h1>
          <p class="drop-shadow-md">Chất lượng, dinh dưỡng và an toàn cho boss yêu của bạn!</p>
        </div>
      </div>
    ...
    <button id="prev" class="absolute top-1/2 left-4 -translate-y-1/2 bg-white/70 hover:bg-white text-purple-700 rounded-full p-3">❮</button>
    <button id="next" class="absolute top-1/2 right-4 -translate-y-1/2 bg-white/70 hover:bg-white text-purple-700 rounded-full p-3">❯</button>
  </section>

  <!-- 🧸 Sản phẩm nổi bật -->
  <section class="container mx-auto p-6 mt-10">
    <h2 class="text-2xl font-bold mb-4 text-purple-700">🌟 Sản phẩm nổi bật</h2>
    <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6">
      <?php if (!empty($products)): ?>
        <?php foreach ($products as $p): ?>
          ...
          ?>
          <div class="relative product-card bg-white shadow-md rounded-xl overflow-hidden hover:shadow-lg transition duration-300">
            <?php if ($hasPromo): ?>
            ...
            <div class="p-4">
              ...
              <?php else: ?>
                <p class="text-purple-700 font-bold"><?= number_format($p['price'], 0, ',', '.') ?>₫</p>
              <?php endif; ?>
              <a href="product_detail.php?id=<?= $p['id'] ?>" class="block text-center bg-pink-300 text-purple-800 py-2 rounded-lg hover:bg-pink-400 mt-3 transition">
                Xem chi tiết
              </a>
            </div>
          </div>
  ...
  <!-- 🎁 Khuyến mãi -->
  <section class="bg-purple-50 py-10">
    <div class="container mx-auto px-6">
      <h2 class="text-2xl font-bold text-purple-700 mb-6 text-center">🎉 Ưu đãi & Khuyến mãi</h2>
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        <?php if (!empty($promotions)): ?>
          <?php foreach ($promotions as $promo): ?>
            <div class="bg-white p-5 rounded-xl shadow hover:shadow-lg transition">
              <h3 class="text-lg font-bold text-purple-700"><?= htmlspecialchars($promo['title'] ?? 'Khuyến mãi đặc biệt 🎁') ?></h3>
              <p class="text-gray-600 mt-2"><?= htmlspecialchars($promo['description'] ?? 'Ưu đãi hấp dẫn cho khách hàng PetShop!') ?></p>
              <p class="text-pink-500 font-semibold mt-3">Giảm <?= htmlspecialchars($promo['discount'] ?? 10) ?>%</p>
            </div>
          <?php endforeach; ?>
        <?php else: ?>
